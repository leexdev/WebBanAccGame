...
//xử lý nhận acc
if($gift == 2){
$db->query("UPDATE `accounts` SET `diamon_ff` = `diamon_ff` + '9999' WHERE `username` = '{$iduser}'");//trừ tiền,cộng kc tích lũy	

}elseif($gift == 1){
        $db->query("UPDATE `accounts` SET `diamon_ff` = `diamon_ff` + '3000' WHERE `username` = '{$iduser}'");//trừ tiền,cộng kc tích lũy	
}elseif($gift == 3){
        $db->query("UPDATE `accounts` SET `diamon_ff` = `diamon_ff` + '178' WHERE `username` = '{$iduser}'");//trừ tiền,cộng kc tích lũy
}elseif($gift == 4){
        $db->query("UPDATE `accounts` SET `diamon_ff` = `diamon_ff` + '80' WHERE `username` = '{$iduser}'");//trừ tiền,cộng kc tích lũy
}elseif($gift == 5){
        $db->query("UPDATE `accounts` SET `diamon_ff` = `diamon_ff` + '0' WHERE `username` = '{$iduser}'");//trừ tiền,cộng kc tích lũy
}elseif($gift == 6){
        $db->query("UPDATE `accounts` SET `diamon_ff` = `diamon_ff` + '85' WHERE `username` = '{$iduser}'");//trừ tiền,cộng kc tích lũy
       
}elseif($gift == 7){
        $db->query("UPDATE `accounts` SET `diamon_ff` = `diamon_ff` + '9999' WHERE `username` = '{$iduser}'");//trừ tiền,cộng kc tích lũy
       
}elseif($gift == 8){
        $db->query("UPDATE `accounts` SET `diamon_ff` = `diamon_ff` + '15000' WHERE `username` = '{$iduser}'");//trừ tiền,cộng kc tích lũy  
...
$msg = $gift == 1  ? 'Chúc mừng bạn đã quay trúng '.$msg :$msg;
